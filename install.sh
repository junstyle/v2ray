...
install_v2ray() {
	...
	v2ray_ver="$(curl -s https://api.github.com/repos/v2fly/v2ray-core/releases/latest | grep 'tag_name' | cut -d\" -f4)"
	...
	if [[ $systemd ]]; then
		cp -f "/tmp/v2ray/systemd/system/*.service" "/lib/systemd/system/"
